### This file is adapted from : https://github.com/joansj/hat/blob/master/src/dataloaders/mixture.py
import os,sys
import os.path
import numpy as np
import torch
...
def get(seed=0,fixed_order=False,pc_valid=0.15):
    ...
    idata=np.arange(8)
    idata=np.array([0,2,1,3,4,5,6,7])
    # if not fixed_order:
    #     idata=list(shuffle(idata,random_state=seed))
    print('Task order =',idata)

    if not os.path.isdir('./data/dat/binary_mixture_8_Data/'):
        os.makedirs('./data/dat/binary_mixture_8_Data')
        # Pre-load
        for n,idx in enumerate(idata):
            if idx==0:
                ...
                dat={}
                dat['train']=datasets.CIFAR10('./data/dat/',train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.CIFAR10('./data/dat/',train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                data[n]['name']='cifar10'
                data[n]['ncla']=10
                for s in ['train','test']:
                ...
                dat={}
                dat['train']=Facescrub('./data/dat/facescrub', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=Facescrub('./data/dat/facescrub', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                ...
                std=[x/255 for x in [63.0,62.1,66.7]]
                dat={}
                dat['train']=datasets.CIFAR100('./data/dat/',train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.CIFAR100('./data/dat/',train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                ...
                std=(0.2752,)
                dat={}
                dat['train']=datasets.MNIST('./data/dat/',train=True,download=True,transform=transforms.Compose([
                    transforms.Pad(padding=2,fill=0),transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.MNIST('./data/dat/',train=False,download=True,transform=transforms.Compose([
                    transforms.Pad(padding=2,fill=0),transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n]={}
                data[n]['name']='mnist'
                data[n]['ncla']=10
                ...
                dat = {}
                dat['train']=datasets.SVHN('./data/dat/',split='train',download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=datasets.SVHN('./data/dat/',split='test',download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                data[n] = {}
                data[n]['name']='svhn'
                data[n]['ncla']=10
                for s in ['train','test']:
                ...
                std=(0.3318,)
                dat={}
                dat['train']=FashionMNIST('./data/dat/fashion_mnist', train=True, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                dat['test']=FashionMNIST('./data/dat/fashion_mnist', train=False, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                data[n]={}
                ...
                dat={}
                dat['train']=TrafficSigns('./data/dat/traffic_signs', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=TrafficSigns('./data/dat/traffic_signs', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                ...
                std=(0.4501,)
                dat={}
                dat['train']=notMNIST('./data/dat/notmnist', train=True, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                dat['test']=notMNIST('./data/dat/notmnist', train=False, download=True, transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
                #mean, std = utils.compute_mean_std_dataset(dat['train']); print(mean,std); sys.exit()
                data[n]={}
            ...
            for s in ['train','test']:
                ...
                torch.save(data[n][s]['x'], os.path.join(os.path.expanduser('./data/dat/binary_mixture_8_Data'),'data'+str(idx)+s+'x.bin'))
                torch.save(data[n][s]['y'], os.path.join(os.path.expanduser('./data/dat/binary_mixture_8_Data'),'data'+str(idx)+s+'y.bin'))
        ...
        for n,idx in enumerate(idata):
            ...
            for s in ['train','test']:
                data[n][s]={'x':[],'y':[]}
                data[n][s]['x'] = torch.load(os.path.join(os.path.expanduser('./data/dat/binary_mixture_8_Data'),'data'+str(idx)+s+'x.bin'))
                data[n][s]['y'] = torch.load(os.path.join(os.path.expanduser('./data/dat/binary_mixture_8_Data'),'data'+str(idx)+s+'y.bin'))
