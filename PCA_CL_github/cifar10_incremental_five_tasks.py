...
def main():
    ...
    parser.add_argument('--train_epoch', type=int, default=10, metavar='N',
                        help='number of epochs for training step (default: 10)')
    ...
    parser.add_argument('--num_out', type=int, default=2, metavar='N',
                        help='Number of output units for each task (default: 2)')
    ...
    running_param_fname   = os.path.join(model_root, 'cifar10_rp_{}.pth'.format(args.file_suffix)) 
    param_fnameB   = os.path.join(model_root, 'cifar10_multi_{}.pth'.format(args.file_suffix))
    param_fnameB_rt= os.path.join(model_root, 'cifar10_multi_rt_{}.pth'.format(args.file_suffix))
    checkpoint_fname = os.path.join(model_root, 'cifar10_ckp_{}.pth'.format(args.file_suffix))
    ...
    #######--------------------------------------------------train------------------------------------###################      
        if (args.train ==1):            
            ...
            for epoch in range(1, args.train_epoch + 1): 
                if (args.split<2 and (epoch == 25 or epoch == 35) ):                
        ...
        if (args.retrain == 1): 
            ...
            if (args.train == 0) :
                model_param = torch.load(param_fnameB) #...............Load old model 
                model.load_state_dict(model_param['state_dict'])
            ...
            for i in range(5): # Here '5' corresponds to 5 convolutional layers 
                ...
                if (args.split == 0):
                    ...
                    optimal_num_filters[i],pca_xform=network_compression_PCA(model.act[list(model.act.keys())[i]],0,out_size,threshold=args.var_kept[i]) 
                    ...
                    ## saving for later use (initialization) 
                    if args.rand_init == 1:
                    ...
                    print ('Projection-Subtraction-PCA at layer {}'.format(i+1))          
                    # PCA Step
                    optimal_num_filters[i]=projection_subtraction_PCA(model.act[list(model.act.keys())[i]],pca_comp_final,lx, i, threshold=args.var_kept[i])
            ...
            for epoch in range(1, args.epoch_list[0] + 1):   
                if ( args.split<2 and  (epoch == int(args.epoch_list[0]*0.2) or epoch == int(args.epoch_list[0]*0.9)) ):
                	adjust_learning_rate(optimizer,epoch,args)
                elif ( args.split>1 and  (epoch == int(args.epoch_list[0]*0.1) or epoch == int(args.epoch_list[0]*0.9))) :
                    adjust_learning_rate(optimizer,epoch,args)
